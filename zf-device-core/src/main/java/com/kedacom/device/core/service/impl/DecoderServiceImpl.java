    ...
    private void handleRes(String str, Integer errCode, String errorMsg) {
        if (errCode != DeviceConstants.SUCCESS) {
            if (StrUtil.isNotBlank(errorMsg)) {
                log.error(str, DeviceErrorEnum.DECODER_OSD_FAILED.getCode(), errorMsg);
                ...
                log.error(str, DeviceErrorEnum.DECODER_OSD_FAILED.getCode(), avIntegrationErrCode.matchErrMsg(errCode));
                throw new DecoderException(DeviceErrorEnum.DECODER_OSD_FAILED.getCode(), avIntegrationErrCode.matchErrMsg(errCode));
            } else {
                log.error(str, DeviceErrorEnum.DECODER_OSD_FAILED.getCode(), DeviceErrorEnum.DECODER_OSD_FAILED.getMsg());

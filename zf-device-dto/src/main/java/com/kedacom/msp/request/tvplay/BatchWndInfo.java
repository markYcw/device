...
public class BatchWndInfo implements Serializable {
    ...
    @ApiModelProperty(value = "必填 - 窗口位置ID", required = true)
    private Integer wnd_id;

    @NotNull(message = "窗口内小窗口索引不能为空")
    @ApiModelProperty(value = "必填 - 窗口内小窗口索引， 从0开始，从上到下，从左到右排布", required = true)
    private Integer wnd_index;

    @NotBlank(message = "信号源ID（UUID）不能为空")
    @ApiModelProperty(value = "必填 - 信号源ID（UUID）", required = true)
    private String chnid;

    @NotNull(message = "信号源类型不能为空")
    @ApiModelProperty(value = "必填 - 信号源类型，0=监控源、1=画面合成源或者会议终端",required = true)

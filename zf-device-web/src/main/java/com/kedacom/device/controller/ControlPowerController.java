...
public class ControlPowerController {
    ...
    public BaseResult<PowerDeviceEntity> addBwPower(@Valid @RequestBody BwPowerDeviceAddVo powerDeviceAddVo, BindingResult br) {
    ...
    public BaseResult<PowerDeviceEntity> updateBwPower(@Valid @RequestBody BwPowerDeviceUpdateVo powerDeviceUpdateVo, BindingResult br) {
        ValidUtils.paramValid(br);
    ...
    public BaseResult<Boolean> deviceDelete(@Valid @RequestBody PowerDeviceDeleteVo powerDeviceDeleteVo, BindingResult br) {
        ValidUtils.paramValid(br);
    ...
    public BaseResult<PageRespVo<List<PowerDeviceListRspVo>>> deviceList(@Valid @RequestBody PowerDeviceListVo powerDeviceListVo, BindingResult br) {
        ValidUtils.paramValid(br);
    ...
    public BaseResult<PowerDeviceListRspVo> getDeviceById(@RequestParam(value = "id") int id) {
    ...
    public BaseResult<List<LanDevice>> lanSearch() {
        try {
            ...
            return BaseResult.succeed(devices);
        } catch (Exception e) {
            log.error("局域网搜索失败：{}", e.getMessage());
            return BaseResult.failed("局域网搜索失败：" + e.getMessage());
        }
    }
    ...
    public BaseResult<PowerLanConfigVO> getPowerConfigByMac(@RequestParam("mac") String mac) {
        try {
            return controlPowerService.getPowerConfigByMac(mac);
        } catch (Exception e) {
            log.error("获取电源的具体配置失败：{}", e.getMessage());
            return BaseResult.failed("获取电源的具体配置失败：" + e.getMessage());
        }
    }

    @ApiOperation(value = "局域网搜索-修改电源设备配置")
    @PostMapping(value = "/device/updatePowerConfigByMac")
    public BaseResult<Boolean> updatePowerConfigByMac(@Valid @RequestBody UpdatePowerLanConfigDTO dto, BindingResult br) {
        ValidUtils.paramValid(br);
        ...
        try {
            ...
            return BaseResult.failed("修改电源配置失败：" + e.getMessage());
        }
    }
    ...
    public BaseResult<Integer> portUpdate(@Valid @RequestBody PowerConfigUpdateVo powerConfigUpdateVo, BindingResult br) {
        ValidUtils.paramValid(br);
    ...
    public BaseResult<List<PowerPortListVo>> portList(@RequestBody PowerConfigListVo powerConfigListVo) {
    ...
    public BaseResult powerStart(@RequestParam(value = "id") int id) throws IOException {
        return controlPowerService.powerStart(id);
    }

    @ApiOperation(value = "获取设备详细信息（针对Bwant-IPM-08）")
    @PostMapping(value = "/device/msg")
    public BaseResult<PowerDeviceMessageVo> deviceMessage(@Valid @RequestBody PowerDeviceMessageReqVo powerDeviceMessageReqVo, BindingResult br) {
        ValidUtils.paramValid(br);
    ...
    public BaseResult<List<PowerChannelStateVo>> deviceChannelState(@Valid @RequestBody PowerDeviceMessageReqVo powerDeviceMessageReqVo, BindingResult br) {
        ValidUtils.paramValid(br);
    ...
    public BaseResult<Boolean> deviceTurn(@Valid @RequestBody PowerDeviceTurnVO vo, BindingResult br) {
        ValidUtils.paramValid(br);
    ...
    public BaseResult<Boolean> deviceTurns(@Valid @RequestBody PowerDeviceTurnsVo powerDeviceTurnsVo, BindingResult br) {
        ValidUtils.paramValid(br);

...
CREATE TABLE `ums_new_device` (
  ...
  `device_type` varchar(30) NOT NULL COMMENT '设备类型，关联ums_device_type表',
  `device_ip` varchar(15) NULL DEFAULT NULL COMMENT '平台IP',
  `device_port` int NULL DEFAULT NULL COMMENT '平台端口',
  `device_notify_ip` varchar(15) NULL DEFAULT NULL COMMENT '设备状态订阅IP',
  `media_ip` varchar(15) NULL DEFAULT NULL COMMENT '媒体调度服务IP',
  `media_port` int NULL DEFAULT NULL COMMENT '媒体调度服务端口',
  `group_id` varchar(32) NULL DEFAULT NULL COMMENT '分组Id，关联ums_new_group表',
  `kafka_addr` varchar(128) NULL DEFAULT NULL COMMENT '设备状态订阅信息',
  `streaming_media_ip` varchar(15) NULL DEFAULT NULL COMMENT '流媒体服务IP',
  `streaming_media_port` int NULL DEFAULT NULL COMMENT '流媒体服务端口',
  `streaming_media_rec_port` int NULL DEFAULT NULL COMMENT '录像服务端口',
  ...
  `update_time` timestamp DEFAULT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '修改时间',
  PRIMARY KEY (`id`) USING BTREE
);

DROP TABLE IF EXISTS `ums_new_device_type`;
CREATE TABLE `ums_new_device_type` (
  `id` varchar(32) DEFAULT NULL COMMENT '数据库Id',
  `code` varchar(5) DEFAULT NULL COMMENT '类型编码',
  `value` varchar(30) DEFAULT NULL COMMENT '设备类型',
...
CREATE TABLE `ums_new_group`  (
  `id` varchar(128) DEFAULT NULL COMMENT '数据库Id',
...
CREATE TABLE `ums_new_sub_device`  (
  ...
  `device_ip` varchar(15) DEFAULT NULL COMMENT 'IP地址',
  ...
  `status` int DEFAULT '1' COMMENT '设备状态',

...
CREATE TABLE `ums_mcu` (
	...
	`mcu_name` VARCHAR ( 128 ) DEFAULT NULL COMMENT '会议平台名称',
	`mcu_ip` VARCHAR ( 50 ) DEFAULT NULL COMMENT '会议平台IP',
	`mcu_port` INT ( 11 ) DEFAULT NULL COMMENT '会议平台端口',
	`mcu_user` VARCHAR ( 128 ) DEFAULT NULL COMMENT '登录会议平台用户名',
	`mcu_password` VARCHAR ( 128 ) DEFAULT NULL COMMENT '登录会议平台密码',
	`mcu_key` VARCHAR ( 32 ) DEFAULT NULL COMMENT '获取token使用',
	`mcu_secret` VARCHAR ( 32 ) DEFAULT NULL COMMENT '获取token使用',
